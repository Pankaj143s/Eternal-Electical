...
// Desktop render function (no changes)
const renderLinkDesktop = ({ label, target }) => (
    <a
        href={target}
        key={label}
...
// Mobile render function: adds onTouchStart and onClick to close the menu on touch
const renderLinkMobile = ({ label, target }, closeMenu) => (
    <a
        href={target}
        key={label}
        onTouchStart={() => closeMenu()}
        onClick={() => closeMenu()}
        className="group relative block text-white uppercase font-medium transition duration-200 hover:text-gray-300"
    >
        {label}
        <span className="absolute left-1/2 -bottom-1 h-[2px] w-full bg-white transform -translate-x-1/2 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-center"></span>
    </a>
        ...
        <nav className="fixed top-0 left-0 w-full z-50 bg-black/70 backdrop-blur-sm">
            <div className="container mx-auto px-4">
                ...
                <div className="hidden md:flex h-16 items-center justify-center gap-8 xl:gap-10 text-sm xl:text-base">
                    {navLinks.slice(0, 4).map(renderLinkDesktop)}
                    {/* Center Logo */}
                    <a href="#banner">
                        <img
                            src="/Eternal_logo.png"
                    ...
                    {navLinks.slice(4).map(renderLinkDesktop)}
                </div>
            </div>
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        {navLinks.map((link) => renderLinkMobile(link, closeMenu))}
                    </motion.div>
                )}
            </AnimatePresence>
        </nav>
